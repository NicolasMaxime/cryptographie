...
void	 shnorr(char *message, mpz_t Ks, mpz_t Kp, key_gpq_t *val){
  ...
  if (ret == 0){
    gmp_printf("Signature vérifiée\n");
  }
  else {
    printf("signature invalide\n");
  }
  gmp_printf("Test de la même signature sur un message bidon :\n");
...
int	main(int ac, char av){
  ...
  printf("<============ TEST EXPMOD ==================>\n");
  test_exponentiation_binaire();
  printf("\n\n<=========== Generation clé ================>\n");
  val = generate_key(Ks, Kp);
  printf("\n\n<=========== Test de la signature de shnorr sur un exemple ==========>\n");
  shnorr("Parce que l'on a pas reussi a generer des clefs, on en a mise par defaut avec des petits nombre premiers", Ks, Kp, val);
  printf("\n\nTest des 100 signature et vérification : \n");
  test_100_signature(Ks, Kp, val);
  return 0;
}
